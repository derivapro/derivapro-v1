...
class Prepayment:
    ...
    def prepayment_probability(self):
        ...
        log_odds = (
            ...
            + self.beta_fico * (self.orig_fico - 700)
...
class Validation:
        ...
        if os.path.exists(self.metadata_file):
            try:
                with open(self.metadata_file, "r") as f:
                    ...
                    self._selected_modeling_columns = metadata.get(
                        "selected_modeling_columns", []
                    )  # Add this line
    ...
    def _recreate_data_split(self):
        ...
        try:
            ...
            if (
                hasattr(self, "_selected_modeling_columns")
                and self._selected_modeling_columns
            ):
                # Validate that all selected columns exist in the training data
                available_cols = [
                    col
                    for col in self._selected_modeling_columns
                ...
                if available_cols:
                    ...
                    logger.debug(
                        "Restored column selection with %s columns",
                        len(available_cols),
                    )
                else:
                    logger.warning("No selected columns found in recreated data split")

        except Exception as e:
            logger.exception("Error recreating data split: %s", e)
            ...
            if col in self.data.columns:
                if conversion_type == "numeric":
                    if not pd.api.types.is_numeric_dtype(self.data[col]):
                        ...
                        logger.debug(
                            "Current dtype for '%s': %s", col, self.data[col].dtype
                        )
                        logger.debug(
                            "Sample values for '%s': %s",
                            col,
                            self.data[col].head().tolist(),
                        )

                        # Simply apply the conversion - let the user decide what's appropriate
                        self.data[col] = pd.to_numeric(self.data[col], errors="coerce")
                        logger.debug("Numeric conversion applied for column '%s'", col)
                    else:
                        logger.debug(
                            "Column '%s' is already numeric, skipping conversion", col
                        )
                elif conversion_type == "categorical":
                    self.data[col] = self.data[col].astype("category")
                    logger.debug("Converted column '%s' to categorical", col)
    ...
    def missing_values(self):
        ...
        return nulls.to_frame("Has Missing Values").to_html(
    ...
    def train_test_split_data(
        self, target_variable, task, train_test_split_ratio, random_state
    ):
        """Split data into training and testing sets"""
        from sklearn.model_selection import train_test_split

        if target_variable == "make selection" or task == "make selection":
            logger.warning("Target variable or task selection was not provided")
            return False

        if target_variable not in self.data.columns:
            logger.warning("Target variable '%s' not found in data", target_variable)
            return False
        ...
        if (
            ...
            available_cols = [
                col
                for col in self._selected_modeling_columns
                if col in self.data.columns and col != target_variable
            ]
            X = self.data[available_cols]
            logger.debug(
                "Using %s selected columns for modeling: %s",
                len(available_cols),
                available_cols,
            )
        else:
            ...
            logger.debug(
                "No column selection applied. Using all %s available columns.",
                len(X.columns),
            )
    ...
    def apply_column_selection(self, selected_columns):
        ...
        modeling_columns = [
            col for col in selected_columns if col != getattr(self, "target", None)
        ]

        # Validate that all selected columns exist in the training data
        missing_cols = [
            col for col in modeling_columns if col not in self.X_train.columns
        ]
        if missing_cols:
            raise ValueError(
                f"Selected columns not found in training data: {missing_cols}"
            )
        ...
        logger.debug(
            "Column selection applied. Using %s columns: %s",
            len(modeling_columns),
            modeling_columns,
        )
        logger.debug("New X_train shape: %s", self.X_train.shape)
        logger.debug("New X_test shape: %s", self.X_test.shape)
    ...
    def feature_selection(self, feature_selection_method="pearson", threshold=0.5):
        ...
        try:
            logger.debug(
                "Starting feature selection with method=%s, threshold=%s",
                feature_selection_method,
                threshold,
            )
            logger.debug("Target variable: %s", self.target)

            # CHECK: Use X_train (which respects column selection) instead of self.data
            if hasattr(self, "X_train") and self.X_train is not None:
                logger.debug("Using X_train data shape: %s", self.X_train.shape)
                feature_data = self.X_train
                target_data = self.y_train
            else:
                logger.debug("Using full data shape: %s", self.data.shape)
                feature_data = self.data.drop(columns=[self.target])
                target_data = self.data[self.target]

            logger.debug("Feature data columns: %s", list(feature_data.columns))
            ...
            if feature_selection_method == "pearson":
                ...
                logger.debug(
                    "Found %s numerical variables for Pearson analysis: %s",
                    len(num_vars),
                    list(num_vars),
                )
                ...
                for col in num_vars:
                    if (
                        feature_data[col].nunique() > 1
                        and not feature_data[col].isna().all()
                        and feature_data[col].std() > 1e-10
                    ):  # Check for near-zero std
                        valid_vars.append(col)

                logger.debug("Valid variables for correlation: %s", valid_vars)

                if not valid_vars:
                    logger.warning("No valid variables found for correlation analysis")
                    return {}, None
                ...
                for col in valid_vars:
                    try:
                        ...
                        if not pd.isna(corr_val):
                            ...
                            logger.debug("Correlation for %s: %s", col, corr_val)
                    except Exception as e:
                        logger.warning(
                            "Error calculating correlation for %s: %s", col, e
                        )
                        continue

                logger.debug("Final correlations: %s", correlations)
                plot = self.create_corr_plot(
                    threshold, valid_vars, feature_data, target_data
                )
                logger.debug("Plot generated successfully: %s", plot is not None)
                ...
                num_vars = feature_data.select_dtypes(include=[np.number]).columns
                logger.debug(
                    "Found %s numerical variables for Spearman analysis: %s",
                    len(num_vars),
                    list(num_vars),
                )
                ...
                for col in valid_vars:
                    try:
                        corr_val = feature_data[col].corr(
                            target_data, method="spearman"
                        )
                        if not pd.isna(corr_val):
                            correlations[col] = corr_val
                    except:
                ...
                return correlations, self.create_spearman_plot(
                    threshold, valid_vars, feature_data, target_data
                )
                ...
                feature_importance = dict(
                    zip(self.X_train.columns, model.feature_importances_)
                )
                logger.debug("Feature importance: %s", feature_importance)
                return feature_importance, self.create_importance_plot(threshold)

        except Exception as e:
            logger.exception("Error in feature_selection: %s", e)
            return None, None

    def create_corr_plot(
        self, threshold, valid_vars=None, feature_data=None, target_data=None
    ):
        ...
        try:
            ...
            if valid_vars is None:
                ...
                for col in num_vars:
                    if (
                        data_to_use[col].nunique() > 1
                        and not data_to_use[col].isna().all()
                        and data_to_use[col].std() > 1e-10
                    ):
                        valid_vars.append(col)
            ...
            if not valid_vars:
                ...
                ax.text(
                    0.5,
                    0.5,
                    "No valid numerical variables for correlation analysis",
                    ha="center",
                    va="center",
                ...
                ax.set_title(
                    f"Correlation Analysis with Target Variable: {self.target}",
                    fontsize=14,
                )
                return self._fig_to_base64(fig)
            ...
            for col in valid_vars:
                try:
                    ...
                    logger.warning("Error calculating correlation for %s: %s", col, e)
                    continue
            ...
            if len(corr_filtered_less) > 0:
                bars_below = ax.bar(
                    x=corr_filtered_less.index,
                    height=corr_filtered_less.values,
                    color="lightgray",
                    alpha=0.6,
            ...
            if len(corr_filtered) > 0:
                bars_above = ax.bar(
                    x=corr_filtered.index,
                    height=corr_filtered.values,
                    color="green",
                    alpha=0.8,
                    label=f"|correlation| ≥ {threshold}",
                )
            ...
            ax.set_title(
                f"Pearson Correlation with Target Variable: {self.target}", fontsize=14
            )
            ...
            logger.exception("Error creating correlation plot: %s", e)
            return None
        ...
        try:
            ...
            if not valid_vars:
                ...
                ax.text(
                    0.5,
                    0.5,
                    "No valid numerical variables for Spearman correlation analysis",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                    fontsize=14,
                ...
                ax.set_title(
                    f"Spearman Correlation Analysis with Target Variable: {self.target}",
                    fontsize=14,
                )
                return self._fig_to_base64(fig)
            ...
            for col in valid_vars:
                try:
                    ...
                    if feature_data is not None and target_data is not None:
                        ...
                        corr_val = self.data[col].corr(
                            self.data[self.target], method="spearman"
                        )
                    if not pd.isna(corr_val):
                        correlations[col] = corr_val
                except Exception as e:
                    logger.warning(
                        "Error calculating Spearman correlation for %s: %s",
                        col,
                        e,
                    )
                    continue
            ...
            if not correlations:
                ...
                ax.text(
                    0.5,
                    0.5,
                    "Unable to calculate Spearman correlations",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                    fontsize=14,
            ...
            ax.set_title(
                f"Spearman Correlation with Target Variable: {self.target}", fontsize=14
            )
            ax.set_xlabel("Features", fontsize=12)
            ...
            logger.exception("Error creating Spearman correlation plot: %s", e)
            return None
        ...
        try:
            if not hasattr(self, "X_train") or self.X_train is None:
                ...
                ax.text(
                    0.5,
                    0.5,
                    "Data must be split before feature importance analysis",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                    fontsize=14,
            ...
            if len(importance_filtered_less) > 0:
                bars_below = ax.bar(
                    x=importance_filtered_less.index,
                    height=importance_filtered_less.values,
                    color="lightgray",
                    alpha=0.6,
                    label=f"Importance < {threshold}",
                )
            if len(importance_filtered) > 0:
                bars_above = ax.bar(
                    x=importance_filtered.index,
                    height=importance_filtered.values,
                    color="blue",
                    alpha=0.8,
                    label=f"Importance ≥ {threshold}",
                )
            ...
            logger.exception("Error in create_importance_plot: %s", e)
            # Return a simple error plot
            fig, ax = plt.subplots(figsize=(12, 8))
            ax.text(
                0.5,
                0.5,
                f"Error creating importance plot: {str(e)}",
                ha="center",
                va="center",
                transform=ax.transAxes,
                fontsize=14,
    ...
    def final_column_selection(self, selected_columns):
        ...
        logger.debug(
            "Final column selection applied. Using %s columns: %s",
            len(modeling_columns),
            modeling_columns,
        )
        logger.debug("New X_train shape: %s", self.X_train.shape)
    ...
    def model_training(self, model_training_method, hyperparameters=None):
        ...
        logger.debug("Original data shape: %s", self.data.shape)
        logger.debug("X_train shape before cleaning: %s", self.X_train.shape)
        logger.debug("y_train shape before cleaning: %s", self.y_train.shape)

        # CHECK: Is column selection applied?
        if hasattr(self, "_selected_modeling_columns"):
            logger.debug(
                "Column selection applied: %s columns selected",
                len(self._selected_modeling_columns),
            )
            logger.debug("Selected columns: %s", self._selected_modeling_columns)
            logger.debug("X_train columns: %s", list(self.X_train.columns))

            # Verify that X_train only contains selected columns
            if set(self.X_train.columns) == set(self._selected_modeling_columns):
                logger.debug("X_train matches selected columns")
            else:
                logger.warning("X_train does NOT match selected columns")
                logger.warning(
                    "Missing selected columns: %s",
                    set(self._selected_modeling_columns) - set(self.X_train.columns),
                )
                logger.warning(
                    "Extra columns in X_train: %s",
                    set(self.X_train.columns) - set(self._selected_modeling_columns),
                )
        else:
            logger.warning("No column selection applied - using all available columns")
        ...
        logger.debug("Checking for NaN values in training data")
        logger.debug("X_train NaN count: %s", X_train.isnull().sum().sum())
        logger.debug("y_train NaN count: %s", y_train.isnull().sum())

        if X_train.isnull().sum().sum() > 0 or y_train.isnull().sum() > 0:
            logger.warning(
                "Found NaN values in training data. Dropping rows with NaN values."
            )
            ...
            logger.debug("Combined dataset shape before cleaning: %s", combined.shape)
            combined_clean = combined.dropna()
            logger.debug(
                "Combined dataset shape after cleaning: %s", combined_clean.shape
            )

            # Check if we have any data left
            if combined_clean.empty:
                raise ValueError(
                    "All data was removed during NaN cleaning. Check your data quality or imputation strategy."
                )

            X_train = combined_clean.drop(columns=[y_train.name])
            y_train = combined_clean[y_train.name]
            logger.debug(
                "After cleaning - X_train shape: %s, y_train shape: %s",
                X_train.shape,
                y_train.shape,
            )
        ...
        # Calculate metrics
        results = {"model": model_name, "hyperparameters": params, "metrics": {}}

        for metric_name, metric_func in metrics.items():
            try:
                if task == "classification" and metric_name in [
                    "Precision",
                    "Recall",
                    "F1 Score",
                ]:
                ...
                logger.warning("Error calculating %s: %s", metric_name, e)
    ...
    def remove_features_by_threshold(
        self, feature_selection_method="pearson", threshold=0.5
    ):
        """
        ...
        try:
            logger.debug(
                "Removing features with method=%s, threshold=%s",
                feature_selection_method,
                threshold,
            )

            # First, get the feature selection results
            feature_results, _ = self.feature_selection(
            ...
            if not feature_results:
                return {
                    "success": False,
                    "error": "No feature selection results available",
                }
            ...
            if feature_selection_method in ["pearson", "spearman"]:
                ...
                features_to_keep = [
                    feature
                    for feature, value in feature_results.items()
            ...
            logger.debug("Features to keep: %s", features_to_keep)
            logger.debug("Features to remove: %s", features_to_remove)
            ...
            self._selected_modeling_columns = (
                features_to_keep + [self.target]
                if hasattr(self, "target") and self.target
                else features_to_keep
            )
            self._save_conversions()

            logger.debug("Updated X_train shape: %s", self.X_train.shape)
            logger.debug("Updated X_test shape: %s", self.X_test.shape)
