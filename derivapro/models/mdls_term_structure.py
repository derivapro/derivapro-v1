...
class YieldTermStructure:
    ...
    def create_rate_helpers(self, start_date):
        ...
        for tenor, rate, source in self.market_rates:
            ...
            if source_lc == "treasury":
                if (tenor.units() == ql.Months) or (
                    tenor.units() == ql.Years and tenor.length() <= 1
                ):
    ...
    def fit_nelson_siegel_curve(self, start_date, curve):
        ...
        zero_rates = [
            curve.zeroRate(d, ql.Actual360(), ql.Continuous).rate()
        ...
        fwd_modeled = -(np.log(df_modeled[1:]) - np.log(df_modeled[:-1])) / (
            ttm[1:] - ttm[:-1]
        )
    ...
    def fit_svensson_curve(self, start_date, curve):
        ...
        def error_fn(params):
            ...
            modeled = np.array([
                svensson_yield(t, beta0, beta1, beta2, beta3, tau1, tau2) for t in ttm
            ])
            return modeled - zero_rates

        result = least_squares(
            error_fn, x0, bounds=([-1, -1, -1, -1, 0.01, 0.01], [1, 1, 1, 1, 10, 10])
        )
    ...
    def yield_curve(self, start_date, fit_selection, method_name, forward_tenor):
        forward_tenor_str = str(
            forward_tenor
        )  # For plotting labels, but we use forward_tenor (Period) in calculations
        ...
        if fit_selection.lower() == "yes":
            ...
            ns_zero, ttm, bootstrapped_zero_rates, r2_zero, r2_df, r2_fwd = (
                self.fit_nelson_siegel_curve(start_date, piecewise_curve)
            )

            # 2.1) Fit Svensson to bootstrapped zero rates
            svensson_zero = self.fit_svensson_curve(start_date, piecewise_curve)[
                0
            ]  # only grab the fitted curve function
            ...
            discount_factors_ns = [
                np.exp(-r * t) for r, t in zip(zero_rates_ns, ttm_grid)
            ]

            fwd_len = ql.Actual365Fixed().yearFraction(
                start_date, start_date + forward_tenor
            )
            forward_rates_ns = [
            ...
            discount_factors_sv = [
                np.exp(-r * t) for r, t in zip(zero_rates_sv, ttm_grid)
            ]

            forward_rates_sv = [
                (svensson_zero(t + fwd_len) * (t + fwd_len) - svensson_zero(t) * t)
                / fwd_len
                for t in ttm_grid
                if (t + fwd_len) <= 30
            ]
            ...
            ttm_pw = np.array([
                ql.Actual365Fixed().yearFraction(start_date, d) for d in dates
            ])
            zero_rates_pw = [
                piecewise_curve.zeroRate(d, ql.Actual360(), ql.Continuous).rate()
                for d in dates
            ]
            ...
            for i in range(len(dates) - fwd_shift):
                fwd = piecewise_curve.forwardRate(
                    dates[i], dates[i + fwd_shift], ql.Actual360(), ql.Continuous
                ).rate()
            ...
            # Zero Rate plot
            axs[0].plot(
                ttm_pw, zero_rates_pw, label="Bootstrapped Zero Rates", marker="o"
            )
            ...
            axs[1].plot(
                ttm_pw,
                discount_factors_pw,
                label="Bootstrapped Discount Factors",
                marker="o",
                color="orange",
            )
            axs[1].plot(
                ttm_grid,
                discount_factors_ns,
                label="Nelson-Siegel Discount Factors",
                marker="x",
                color="red",
            )
            axs[1].plot(
                ttm_grid,
                discount_factors_sv,
                label="Svensson Discount Factors",
                marker="+",
                color="blue",
            )
            ...
            axs[2].plot(
                forward_ttm_pw,
                forward_rates_pw,
                label="Bootstrapped Forward Rates",
                marker="o",
                color="green",
            )
            axs[2].plot(
                forward_ttm_ns,
                forward_rates_ns,
                label="Nelson-Siegel Forward Rates",
                marker="x",
                color="darkgreen",
            )
            axs[2].plot(
            ...
            zero_rates = [
                curve.zeroRate(d, ql.Actual360(), ql.Continuous).rate() for d in dates
            ]
            ...
            for i in range(len(dates) - fwd_len):
                fwd = curve.forwardRate(
                    dates[i], dates[i + fwd_len], ql.Actual360(), ql.Continuous
                ).rate()
            ...
            plt.plot(
                ...
                marker="o",
                color="green",
                label=f"{forward_tenor_str} Forward Rate",
            )
