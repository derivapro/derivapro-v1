...
class BinomialTreeEngineCRR:
    ...
    def _build_dt_schedule(self):
        ...
        # === PATCH (CRR equivalence): if NO dividends, force classic dt grid ===
        if len(div_times) == 0:
    ...
    def _setup_forward_tree(self):
        ...
        # === PATCH (CRR equivalence): if no dividends, use spot for American CRR ===
        if not getattr(self, "_internal_divs", []):
            
            F[0, 0] = self.S0 # For pure CRR case (no dividends), initial forward node should be spot for correct American pricing
        else:
            prod_prop = 1.0
            cash_contrib = 0.0
            for d in self._internal_divs:
        ...
        # ... rest of forward tree diffusion logic unchanged ...
...
if __name__ == "__main__":
    ...
    price_new_binomial_tree = engine.price()
    print(f"[Sanity] New Binomial Tree price (no divs): {price_new_binomial_tree:.5f}")
