...
def forwards():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "forwards.md"
    )
    with open(readme_path, "r") as readme_file:
        content = readme_file.read()
    md_content = markdown.markdown(content)

    forward_price_results = forward_pL = margin_requirement = (
    ...
    if request.method == "POST":
        ...
        form_data = {
            ...
            "convenience_yield": float(request.form["convenience_yield"])
            if "convenience_yield" in request.form
            else 0,
        ...
        if action == "sensitivity":
            try:
                ...
                settlement_date = datetime.strptime(
                    form_data["settlement_date"], "%Y-%m-%d"
                )
                forwards_analysis = ForwardsAnalysis(
                    ticker,
                    risk_free_rate,
                    dividend_yield,
                    convenience_yield,
                ...
                if variable == "risk_free_rate":
                    forward_sensitivity_analysis_results = (
                        forwards_analysis.analyze_variable_sensitivity(
                            variable, step_range, num_steps
                        )
                    )
                    forwards_analysis.plot_sensitivity_analysis(
                        variable, step_range, num_steps
                    )
                    ...
                    model_selection == "cost_carry_model"
                    and variable == "convenience_yield"
                ):
                    forward_sensitivity_analysis_results = (
                        forwards_analysis.analyze_variable_sensitivity(
                            variable, step_range, num_steps
                ...
                plot_filename = f"forwards_{variable}_{step_range}_sensitivity_plot_{uuid.uuid4().hex}.png"
    ...
    return render_template(
        "forwards.html",
        form_data=form_data,
        forward_price_results=forward_price_results,
        forward_pL=forward_pL,
        margin_requirement=margin_requirement,
        forward_sensitivity_analysis_results=forward_sensitivity_analysis_results,
...
def futures():
    ...
    futures_price_results = futures_pL = margin_dict = mark_to_market_results = (
    ...
    if request.method == "POST":
        ...
        margin_dict = future_model.calculate_margin_requirement(
        ...
        if action == "sensitivity":
            try:
                ...
                if variable == "risk_free_rate":
                    future_sensitivity_analysis_results = (
                        futures_analysis.analyze_variable_sensitivity(
                            variable, step_range, num_steps
                        )
                    )
                    futures_analysis.plot_sensitivity_analysis(
                        variable, step_range, num_steps
                    )
                elif variable == "dividend_yield":
                    future_sensitivity_analysis_results = (
                ...
                plot_filename = f"futures-{variable}-{step_range}_sensitivity_plot_{uuid.uuid4().hex}.png"
    ...
    return render_template(
        "futures.html",
        form_data=form_data,
        futures_price_results=futures_price_results,
        mark_to_market_results=mark_to_market_results,
        futures_pL=futures_pL,
        margin_requirement=margin_dict,
        future_sensitivity_analysis_results=future_sensitivity_analysis_results,
