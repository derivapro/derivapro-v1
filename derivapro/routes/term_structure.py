...
def calculate_term_structure():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "term_structure.md"
    )
    ...
    form_data = {}
    plot = r2_ns_zero = r2_ns_df = r2_ns_fwd = r2_sv_zero = r2_sv_df = r2_sv_fwd = (
        plot1
        ...
        if action == "parallel":
            try:
                ...
                if parallelShockValue == "+50":
                    ...
                    shocked_sofr_rates = [
                        (tenor, rate + 0.005) for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [(tenor, rate + 0.005) for tenor, rate in selected_swap_rates]
                        if selected_swap_rates
                        else []
                    )
                    ...
                    shocked_sofr_rates = [
                        (tenor, rate + 0.01) for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [(tenor, rate + 0.01) for tenor, rate in selected_swap_rates]
                        if selected_swap_rates
                        else []
                    )
                    ...
                    shocked_sofr_rates = [
                        (tenor, rate + 0.015) for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [(tenor, rate + 0.015) for tenor, rate in selected_swap_rates]
                        if selected_swap_rates
                        else []
                    )
                    ...
                    shocked_sofr_rates = [
                        (tenor, rate + 0.02) for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [(tenor, rate + 0.02) for tenor, rate in selected_swap_rates]
                        if selected_swap_rates
                        else []
                    )
                    ...
                    shocked_sofr_rates = [
                        (tenor, rate + 0.025) for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [(tenor, rate + 0.025) for tenor, rate in selected_swap_rates]
                        if selected_swap_rates
                        else []
                    )
                    ...
                    shocked_sofr_rates = [
                        (tenor, rate + 0.03) for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [(tenor, rate + 0.03) for tenor, rate in selected_swap_rates]
                        if selected_swap_rates
                        else []
                    )
                ...
                ts_shocked.append_market_rates(
                    shocked_treasury_rates, source="treasury"
                )
                ...
                safe_parallelShockValue = (
                    parallelShockValue.replace("+", "plus").replace("-", "minus")
                    if parallelShockValue
                    else "none"
                )
                ...
                fig1 = ts_shocked.yield_curve(
                    start_date, fit_selection, method_name, forward_tenor
                )
                ...
                if fit_selection.lower() == "yes":
                    _, _, _, r2_zero_ns1, r2_df_ns1, r2_fwd_ns1 = (
                        ts_shocked.fit_nelson_siegel_curve(start_date, curve1)
                    )
            ...
            try:
                ...
                def apply_steepener_shock(tenor, rate):
                    ...
                    shock = (
                        -0.65 * int(nonParallelAbsoluteShock) * np.exp(-tenor_value / 4)
                    ) + (
                        0.9
                        * int(nonParallelAbsoluteShock)
                        * (1 - np.exp(-tenor_value / 4))
                    )
                ...
                def apply_flattener_shock(tenor, rate):
                    tenor_value = tenor_translation[str(tenor)]
                    shock = (
                        0.8 * int(nonParallelAbsoluteShock) * np.exp(-tenor_value / 4)
                    ) - (
                        0.6
                        * int(nonParallelAbsoluteShock)
                        * (1 - np.exp(-tenor_value / 4))
                ...
                if nonParallelShockValue == "Steepener":
                    ...
                    shocked_sofr_rates = [
                        (tenor, apply_steepener_shock(tenor, rate))
                        for tenor, rate in selected_sofr_rates
                    ]
                    shocked_swap_rates = (
                        [
                            (tenor, apply_steepener_shock(tenor, rate))
                            for tenor, rate in selected_swap_rates
                        ]
                        if selected_swap_rates
                        else []
                    )
                ...
                # 1. Create filename first
                safe_nonParallelShockValue = (
                    nonParallelShockValue if nonParallelShockValue else "none"
                )
                safe_nonParallelAbsoluteShock = (
                    nonParallelAbsoluteShock if nonParallelAbsoluteShock else "none"
                )
                ...
                # Conditionally calculate fit stats only if fit_selection is "yes"
                if fit_selection.lower() == "yes":
                    _, _, _, r2_zero_ns2, r2_df_ns2, r2_fwd_ns2 = (
                        ts_shocked.fit_nelson_siegel_curve(start_date, curve2)
                    )
            ...
            try:
                ...
                plot_path = os.path.join(
                    current_app.root_path, "static", "plots", plot_filename
                )
                ...
                if fit_selection.lower() == "yes":
                    ...
                    _, _, _, r2_zero_sv, r2_df_sv, r2_fwd_sv = ts.fit_svensson_curve(
                        start_date, curve
                    )
