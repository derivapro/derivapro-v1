...
def american_options():
    ...
    return render_template(
        ...
        scenario_results=scenario_results,
        md_content=md_content,
        gpt_rpbl_assessment=session.get("risk_pl_results", {}).get(
            "gpt_rpbl_assessment"
        ),
        gpt_sensitivity_assessment=session.get("sensitivity_results", {}).get(
            "gpt_sensitivity_assessment"
        ),
        gpt_convergence_assessment=session.get("convergence_results", {}).get(
            "gpt_convergence_assessment"
        ),
        action=action,
    )
