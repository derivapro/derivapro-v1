...
def creditDefaultSwaps():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "credit_default_swaps.md"
    )
    ...
    if request.method == "POST":
        ...
        if action == "sensitivity":
            ...
            try:
                ...
                cds_stressed = CreditDefaultSwap(
                    nominal,
                    stressed_spread,
                    stressed_rec,
                    stressed_rate,
    ...
    return render_template(
        ...
        cds_analysis_results=cds_analysis_results,
...
@credit_derivatives_bp.route(
    "/synthetic_collateralized_debt_obligation", methods=["GET", "POST"]
)
def syntheticCDO():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "synthetic_CDO.md"
    )
    with open(readme_path, "r") as readme_file:
        content = readme_file.read()
    md_content = markdown.markdown(content)
    ...
    if request.method == "POST":
        ...
        for i in range(num_contracts):
            ...
            form_data[f"selected_tenor_{i + 1}"] = request.form[
                f"selected_tenor_{i + 1}"
            ]
            ...
            calendar = (
                ...
                if calendar_type == "American"
                    ...
                    ql.TARGET()
                    if calendar_type == "TARGET"
                    else (
                        ql.UnitedKingdom()
                        if calendar_type == "UnitedKingdom"
                        else ql.China()
                    )
                )
            )
            ...
            # Define Side (Seller/Buyer)
            side = (
                ql.Protection.Seller if side_type == "Seller" else ql.Protection.Buyer
            )
            ...
            selected_period, payment_frequency_ql, period_fraction = tenor_mapping[
                selected_tenor
            ]
        ...
        synthetic_cdo = SyntheticCDO(
            cds_list=[cds["instance"] for cds in cds_list], tranches=tranches
        )
        ...
        if action == "sensitivity":
            ...
            try:
                ...
                baseline_CDO = (
                    synthetic_cdo.calculate_synthetic_cdo()
                )  # This should calculate the baseline CDO
...
def creditLinkedNotes():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "credit_linked_notes.md"
    )
    with open(readme_path, "r") as readme_file:
        content = readme_file.read()
    md_content = markdown.markdown(content)

    fr_bond_results = cds_results_fixed = cds_results_float = cln_pricing_fixed = (
        cln_pricing_float
    ) = fram_bond_results = fl_bond_results = flam_bond_results = None
    cln_sensitivity_analysis_results_fixed = cln_sensitivity_analysis_results_float = (
        baseline_CLN
    ) = stressed_CLN = baseline_CLN_float = stressed_CLN_float = None
    form_data = {}
        ...
        if tab_selected == "fixed-tab":
            ...
            if amort_selection == "No":
                ...
                fram_bond_results = fixed_bond.fixed_rate_amortizing(
                    issue_date, maturity_date, tenor, coupon_rate, notionals
                )

            cln_pricing = CLNPricingFixed(bond_params=bond_params, cds_instance=cds)

            cln_pricing_fixed = cln_pricing.calculate_cln_price_fixed(
                issue_date,
                maturity_date,
                tenor,
                coupon_rate,
            ...
            if action == "fixed-sensitivity":
                try:
                    ...
                    if amort_selection == "No":
                        if variable == "risk_free":
                            ...
                            sensitivity_analysis = CLNSensitivityAnalysis(
                                bond_params=bond_params,
                                cds_instance=cds_class,
                                bond_results=fr_bond_results,
                            )
                            # results = sensitivity_analysis.analyze_variable_sensitivity(variable, range_span, num_steps)
                            sensitivity_analysis.plot_sensitivity_analysis(
                                variable, range_span, num_steps
                            )
                        ...
                        if variable == "risk_free":
                            ...
                            sensitivity_analysis = CLNSensitivityAnalysis(
                                bond_params=bond_params,
                                cds_instance=cds_class,
                                bond_results=fram_bond_results,
                            )
                            # results = sensitivity_analysis.analyze_variable_sensitivity(variable, range_span, num_steps)
                            sensitivity_analysis.plot_sensitivity_analysis(
                                variable, range_span, num_steps
                ...
                try:
                    ...
                    if amort_selection == "No":
                        ...
                        cln_pricing_baseline = CLNPricingFixed(
                            bond_params=bond_params, cds_instance=cds
                        )
                        baseline_CLN = cln_pricing_baseline.calculate_cln_price_fixed(
                            issue_date,
                            maturity_date,
                            tenor,
                            coupon_rate,
                        ...
                        cln_pricing_stressed = CLNPricingFixed(
                            bond_params=bond_params, cds_instance=cds_stressed
                        )
                        stressed_CLN = cln_pricing_stressed.calculate_cln_price_fixed(
                            issue_date,
                            maturity_date,
                            tenor,
                            coupon_rate,
            ...
            cln_pricing = CLNPricingFloat(
                cds_instance=cds, bond_params_float=bond_params_float
            )
            ...
            cln_pricing_float = cln_pricing.calculate_cln_price_float(
                shocks,
                issue_date,
                maturity_date,
                tenor,
            ...
            if action == "float-sensitivity":
                try:
                    ...
                    if amort_selection_float == "No":
                        if variable == "risk_free":
                            ...
                            sensitivity_analysis = CLNSensitivityAnalysis(
                                bond_params=bond_params_float,
                                cds_instance=cds_class,
                                bond_results=fl_bond_results,
                            )
                            sensitivity_analysis.plot_sensitivity_analysis(
                                variable, range_span, num_steps
                            )
                        ...
                        if variable == "risk_free":
                            ...
                            sensitivity_analysis = CLNSensitivityAnalysis(
                                bond_params=bond_params_float,
                                cds_instance=cds_class,
                                bond_results=flam_bond_results,
                            )
                            sensitivity_analysis.plot_sensitivity_analysis(
                                variable, range_span, num_steps
                            )
                ...
                try:
                    ...
                    recovery_change = float(
                        request.form.get("recovery_scenario_float", 0)
                    )
                    ...
                    if amort_selection_float == "No":
                        ...
                        cln_pricing_baseline = CLNPricingFloat(
                            bond_params_float=bond_params_float, cds_instance=cds_class
                        )
                        baseline_CLN_float = (
                            cln_pricing_baseline.calculate_cln_price_float(
                                shocks,
                                issue_date,
                                maturity_date,
                                tenor,
                        ...
                        cln_pricing_stressed = CLNPricingFloat(
                            bond_params_float=bond_params_float,
                            cds_instance=cds_stressed,
                        )
                        stressed_CLN_float = (
                            cln_pricing_stressed.calculate_cln_price_float(
                                shocks,
                                issue_date,
                                maturity_date,
                                tenor,
                        ...
                        cln_pricing_baseline = CLNPricingFloat(
                            bond_params_float=bond_params_float, cds_instance=cds
                        )
                        baseline_CLN_float = (
                            cln_pricing_baseline.calculate_cln_price_float(
                                shocks,
    ...
    return render_template(
        ...
        cds_results_fixed=cds_results_fixed,
        cds_results_float=cds_results_float,
        cln_pricing_fixed=cln_pricing_fixed,
        cln_pricing_float=cln_pricing_float,
        cln_sensitivity_analysis_results_fixed=cln_sensitivity_analysis_results_fixed,
        cln_sensitivity_analysis_results_float=cln_sensitivity_analysis_results_float,
        baseline_CLN=baseline_CLN,
        stressed_CLN=stressed_CLN,
        baseline_CLN_float=baseline_CLN_float,
        stressed_CLN_float=stressed_CLN_float,
    )
