...
def extract_market_data():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "extract_market_data.md"
    )
    ...
    if request.method == "POST":
        ...
        # --- SOFR Compounded Rates extraction ---
            ...
            sofr_form_data = {
                ...
                "compounding_method": request.form.get(
                    "compounding_method", "Compounded"
                ),
                "compounding_frequency": request.form.get(
            ...
            if start_date and selected_tenors:
                ...
                history_start_date = as_of_date - datetime.timedelta(
                    days=30 * max_months
                )
                ...
                if data and "refRates" in data:
                    rates = [
                        (
                            ql.DateParser.parseISO(entry["effectiveDate"]),
                            entry["percentRate"] / 100.0,
                        )
                    ...
                    for tenor_str in selected_tenors:
                        if "M" in tenor_str:
                            period = ql.Period(
                                int(tenor_str.replace("M", "")), ql.Months
                            )
                        elif "Y" in tenor_str:
                            period = ql.Period(
                                int(tenor_str.replace("Y", "")), ql.Years
                            )
                        else:
                            continue
                        try:
