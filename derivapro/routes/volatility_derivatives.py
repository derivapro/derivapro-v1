# -*- coding: utf-8 -*-
"""
Created on Sun Jun  9 00:47:03 2024
...
@volatility_derivatives_bp.route("/", methods=["GET", "POST"])
def volatility_derivatives():
    return render_template("volatility_derivatives.html")


@volatility_derivatives_bp.route("/variance-swaps", methods=["GET", "POST"])
def variance_swaps():
    readme_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "variance_swaps.md"
    )
    with open(readme_path, "r") as readme_file:
        content = readme_file.read()
    md_content = markdown.markdown(content)

    variance_notional = realized_variance = settlement_amount = (
        simulated_settlement_amount
    ) = expected_real_variance = current_value = None
    ...
    if request.method == "POST":
        form_data = {
            ...
            "strike_vol": request.form["strike_vol"],
            "new_strike_vol": request.form["new_strike_vol"],
            "vega_notional": request.form["vega_notional"],
        ...
        logger.debug("Created variance swap object: %s", variance_swap)
        ...
        simulated_settlement_amount, expected_real_variance = (
            variance_swap.simulated_settlement_amount(position)
        )
