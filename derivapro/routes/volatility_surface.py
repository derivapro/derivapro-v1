...
def fetch_option_chain(symbol, num_expirations=5):
    ...
    for exp in expirations:
        try:
            ...
            logger.warning("Error fetching options for expiration %s: %s", exp, e)
            continue
    ...
    # Convert to numeric
    vol_df["impliedVolatility"] = pd.to_numeric(
        vol_df["impliedVolatility"], errors="coerce"
    )
...
def generate_volatility_surface(vol_df):
    vol_table = vol_df.pivot_table(
        index="moneyness", columns="expiration", values="impliedVolatility"
    )
...
def volatility_surface():
    ...
    vol_table_html = (
        ...
        if isinstance(vol_table, pd.DataFrame) and not vol_table.empty
        else None
    )
...
def download_vol_surface():
    ...
    try:
        ...
        response.headers.set(
            "Content-Disposition",
            f"attachment; filename=volatility_surface_{symbol}.csv",
        )
